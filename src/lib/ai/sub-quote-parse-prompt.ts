 ...
 *   - The project's existing budget category roster (name + section)
...
5. For allocations:
   - budget_category_name MUST be an EXACT match to one of the existing category names provided. Do not invent category names. Do not case-shift.
   - If the scope text does not clearly match any existing category, leave allocations empty. Do not pre-fill a guess — operators strongly prefer to allocate manually rather than correct a wrong AI guess.
   ...
   - Mark each allocation's confidence honestly: "high" (scope literally names the category), "medium" (scope thematically matches the category), "low" (don't include — leave for operator).
