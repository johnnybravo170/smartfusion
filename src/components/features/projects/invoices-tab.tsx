...
function MilestoneForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border bg-muted/30 p-4">
      <div>
        <label htmlFor="milestone-label" className="mb-1 block text-xs font-medium">
          Milestone Label
        </label>
        <Input
          id="milestone-label"
...
export function InvoicesTab({
  ...
  const drawCount = invoices.filter(
    (inv) => inv.status !== 'void' && inv.customer_note !== 'Final invoice',
  ).length;
  const defaultMilestoneLabel = `Draw #${drawCount + 1}`;
  ...
  return (
    <div className="space-y-6">
      ...
      {showMilestoneForm && (
        <MilestoneForm
          projectId={projectId}
          defaultLabel={defaultMilestoneLabel}
          onDone={() => setShowMilestoneForm(false)}
        />
      )}
                ...
                {invoices.map((inv) => (
                  ...
                  >
                    <td className="px-3 py-2 font-medium">
                      {inv.customer_note || `Invoice #${inv.id.slice(0, 8)}`}
                    </td>
                    ...
                    <td className="px-3 py-2 text-right font-medium">
                      {formatCurrency(inv.amount_cents + inv.tax_cents)}
                    </td>
