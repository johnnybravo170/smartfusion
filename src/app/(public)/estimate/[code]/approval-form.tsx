...
import { approveEstimateAction, declineEstimateAction } from '@/server/actions/estimate-approval';

export function EstimateApprovalForm({ approvalCode }: { approvalCode: string }) {
  ...
  async function handleApprove() {
    ...
    setLoading(true);
    setError(null);
    const result = await approveEstimateAction(approvalCode, name.trim());
    if (!result.ok) {
      setError(result.error);
      setLoading(false);
      return;
    }
    setResultMessage('Estimate approved. Your contractor has been notified.');
    setMode('done');
    setLoading(false);
  }
  ...
  async function handleDecline() {
    setLoading(true);
    setError(null);
    const result = await declineEstimateAction(approvalCode, declineReason.trim() || undefined);
    if (!result.ok) {
      setError(result.error);
      setLoading(false);
      return;
    }
    setResultMessage('Estimate declined. Your contractor has been notified.');
    setMode('done');
    setLoading(false);
  }
      ...
      {mode === 'approve' ? (
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Type your name below to approve this estimate.
          </p>
          <input
            type="text"
            value={name}
