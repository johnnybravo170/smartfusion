...
test.describe.skip.serial('quotes lifecycle: create → send → accept → convert to job', () => {
  ...
  test('full quote lifecycle with manual surface entry', async ({ page }) => {
    ...
    await expect(page.locator('[data-slot="quote-status-badge"]').first()).toHaveText('Accepted', {
      timeout: 10_000,
    });
